  ...
  <div class="ibar_cart_group ibar_cart_product">
      ...
      <?php if(is_array($all)): $i = 0; $__LIST__ = $all;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><span>
          ...
          <tr>
             <td id="getChange3">
                ...
                <?php else: ?>
                  <button a="<?php echo ($v["id"]); ?>" b="<?php echo ($v["price"]); ?>" onclick="doMinus(this)" class="btn btn-success btn-xs" style="width:21px;font-weight:bold;">-</button>
                  &nbsp;&nbsp;<span><?php echo ($v["num"]); ?></span>&nbsp;&nbsp;
                  <button a="<?php echo ($v["id"]); ?>" b="<?php echo ($v["price"]); ?>" onclick="doAdd(this)" class="btn btn-success btn-xs" style="font-weight:bold;">+</button><?php endif; ?>
            </td>
          </tr>
          <tr>
            <td>
          ...
          <tr>
           <td id="getChange1">
              ...
              <?php else: ?>
              <button a="<?php echo ($v["foodsid"]); ?>" onclick="doDrop(this)" class="btn btn-danger" style="font-size:10px;font-weight:bold">退选餐点</button><?php endif; ?>     
           </td>
          </tr>
    ...
    <div class="cart_handler_header">
      ...
      <span class="cart_handler_right" style="margin-right:20px"=>
        ￥<span id="total"><?php echo ($list["total"]); ?></span>.00
      </span>
    </div>
    <span id="getChange2">
        ...
        <?php else: ?>
          <button a="<?php echo ($v["desk"]); ?>" id="checkout2" class="cart_go_btn">
           结账
          </button><?php endif; ?>      
    </span>
</div>
    ...
    function doAdd(obj){
        ...
        $.ajax({
           ...
           success:function(data){
               ...
               //总额加单价
               var c = $('#total').html();//总额
               $('#total').html(parseInt(c)+parseInt(price));
