									...
									<table id="sample-table-1" class="table table-striped table-bordered table-hover">
										...
										<tbody>
										  <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
												<td style="line-height:30px"><?php echo ($v["orderid"]); ?></td>
												<td style="line-height:30px"><?php echo ($v["desk"]); ?></td>
												<td style="line-height:30px"><?php echo ($v["name"]); ?></td>
												<td style="line-height:30px"><?php echo ($v["num"]); ?></td>
												<td>
												<?php if($v["status"] == 0): ?><span class="label label-sm label-warning">已下单</span>
												<?php else: ?>	
													<span class="label label-sm label-success">已下锅</span><?php endif; ?>											
												</td>
												<td style="line-height:30px"><?php echo date('Y-m-d H:i:s',$v['addtime']); ?></td>
												<td style="line-height:30px">
												<?php if($v["status"] == 0): ?><a href="<?php echo U('Admin/Cooking/edit',array('id'=>$v['id'],'status'=>1));?>" class="btn btn-xs btn-success">
															开始烹饪
														</a>
												<?php else: ?>	
